...
from ai_job_image import generate
...
def main() -> None:
    ...
    for idx, source_item in enumerate(items):
        ...
        try:
            ...
            elif "job-featured-image" in content:
                raise RuntimeError("Featured image block exists but old image URL was not found")
            else:
                raise RuntimeError("Published article content has no featured image block")
